...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        if(this.size==0){
            ...
            boolean flag=false;
            while(p!=null&&p!=this.sentinel){
                if(p.item.compareTo(item)>=0){
                    Node newN=new Node(item,p.prev,p);
                    p.prev=newN;
                    newN.prev.next=newN;
                    flag=true;
                    break;
    ...
    public DLList<T> mergeSort() {
        ...
        Node p2=sentinel.next;
        while(p2.next!=sentinel&&p2.next.next!=sentinel){
            oneHalf.addLast(p1.item);
            p1=p1.next;
            p2=p2.next.next;
        }
        oneHalf.addLast(p1.item);
        Node x2=p1.next;
        p1.next=this.sentinel;
        this.sentinel.prev=p1;
        while(x2!=this.sentinel){
            otherHalf.addLast(x2.item);
            x2=x2.next;
        }
        DLList<T> tdlList = oneHalf.mergeSort();
        DLList<T> tdlList1 = otherHalf.mergeSort();
        DLList<T> merge = tdlList.merge(tdlList1);
        return merge;
    }
    ...
    public DLList<T> quicksort() {
        ...
        // TODO: YOUR CODE HERE
        Node left=this.sentinel.next;
        Node right=this.sentinel.prev;
//        while(left!=right.next){
//            if(left.item.compareTo(pivot)==0) equalElements.addLast(pivot);
//            else if(left.item.compareTo(pivot)<0) smallElements.addLast(left.item);
//            else largeElements.addLast(left.item);
//            left=left.next;
//        }
//
...
//        return smallElements;
        while(left!=right){
            while(left!=right&&right.item.compareTo(pivot)>0){
                right=right.prev;
            }
            left.item=right.item;
            while(left!=right&&left.item.compareTo(pivot)<=0){
                left=left.next;
            }
        ...
        DLList<T> quicksort1 = smallElements.quicksort();
        DLList<T> quicksort = largeElements.quicksort();
        quicksort1.append(equalElements);
        quicksort1.append(quicksort);
        return quicksort1;
    }
